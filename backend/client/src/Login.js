...
function Login() { 
  ...
  const onSubmit = (data) => {
    setUsr(data);
    loadData(data);
    axios.post('https://bhav-ecommerceapp.herokuapp.com/auth',data).then(res => { 
      if(res.data)
      {
        setUsed(false);
  ...
  const loadData = (data) => {
    axios.post("https://bhav-ecommerceapp.herokuapp.com/fetchCart",data)
        .then(res => { 
            dispatch(cartAdd(res.data));
        })
        .catch(error => console.log("Error in getting cart"));
        
    axios.post("https://bhav-ecommerceapp.herokuapp.com/fetch",data)
        .then(res => { 
